...
public class AuthController : ControllerBase
{
    ...
    public async Task<IActionResult> ExternalGoogleCode([FromBody] ExternalGoogleCodeRequest request)
    {
        ...
        try
        {
            ...
            {
                // Persist detailed token exchange response to a local log file for debugging (safe on private deployment)
                try
                {
                    var logsDir = Path.Combine(AppContext.BaseDirectory, "Logs");
                    Directory.CreateDirectory(logsDir);
                    var fileName = $"google_token_error_{DateTime.UtcNow:yyyyMMdd_HHmmss_fff}.log";
                    ...
                    await SysFile.WriteAllTextAsync(filePath, JsonSerializer.Serialize(details, new JsonSerializerOptions { WriteIndented = true }));
                }
                catch
                {
