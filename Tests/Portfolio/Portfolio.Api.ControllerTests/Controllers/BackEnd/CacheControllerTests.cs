...
namespace Portfolio.Api.ControllerTests.Controllers.BackEnd
{
    public class CacheControllerTests
    {
        ...
        public async Task ClearCache_WhenNoCacheIsSelected_ReturnsBadRequestWithoutCallingService()
        {
            ...
            badRequest.Value.Should().Be("At least one cache must be selected.");

            _cacheService.Verify(service => service.Clear(It.IsAny<bool>(), It.IsAny<bool>(), It.IsAny<bool>()), Times.Never);
        }
        ...
        public async Task ClearCache_WhenServiceSucceeds_ReturnsOkWithMappedResult()
        {
            ...
            var response = okResult.Value.Should().BeOfType<Portfolio.Contracts.Responses.CacheClearResult>().Subject;
