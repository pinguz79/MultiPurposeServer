...
namespace Portfolio.Api.ControllerTests.Controllers.BackEnd
{
    public class FotoControllerTests
    {
        ...
        [Fact]
        public async Task Update_WhenRequestIsNull_ReturnsBadRequestWithoutCallingService()
        {
            // Arrange
            var photoId = Guid.NewGuid();

            // Act
            var result = await _controller.Update(photoId, null!);

            // Assert
            result.Should().BeOfType<BadRequestResult>();

            _fotoService.Verify(service => service.UpdateDescription(It.IsAny<Guid>(), It.IsAny<string?>()), Times.Never);
        }
        ...
        [Fact]
        public async Task Update_WhenDescriptionIsNull_CallsServiceWithNull()
        {
            // Arrange
            var photo = CreatePhoto("Portrait.jpg");

            var request = new UpdatePhotoRequest(null);

            _fotoService.Setup(service => service.UpdateDescription(photo.Id, null)).ReturnsAsync(photo);
            ...
            _fotoService.Verify(service => service.UpdateDescription(photo.Id, null), Times.Once);
        }
