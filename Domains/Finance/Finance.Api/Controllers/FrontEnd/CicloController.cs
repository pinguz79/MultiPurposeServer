...
namespace Finance.Api.Controllers.FrontEnd
{
    ...
    [ApiController]
    public class CicloController(IMovimentoService movimentoService) : FinanceFrontEndControllerBase
    {
        [HttpGet("List")]
        public async Task<IActionResult> GetList(string contoName, [FromQuery] int? month, [FromQuery] int? year)
        {
            ...
            try
            {
                return Ok(month is null && year is null
                    ? await movimentoService.GetCurrentCycleTimeline(contoName)
                    : await movimentoService.GetCycleTimeline(contoName, selectedMonth, selectedYear));
            }
            ...
            {
                return Conflict(new ProblemDetails { Title = "Invalid card configuration", Detail = exception.Message });
            }
