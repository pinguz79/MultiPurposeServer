...
Il test di connessione non crea, modifica o elimina file remoti. Il test di trasferimento elenca prima la root remota, quindi usa `curl` sul runner con FTPS implicito sulla porta 990, in modalità passiva classica senza EPSV, e opera esclusivamente su una sentinella temporanea dal nome univoco `modelbook.cloud/codex-aruba-ftps-transfer-<guid>.txt`, della quale verifica il contenuto prima di tentarne sempre la cancellazione. Entrambi possono essere eseguiti con il piano di esempio proposto dal workflow.
