...
Il test di connessione non crea, modifica o elimina file remoti. Il test di trasferimento usa esclusivamente `.codex-altervista-ftps-transfer-test.txt` e lo elimina nel blocco di pulizia dopo averne verificato il contenuto. Il canale di controllo usa la validazione .NET della fingerprint revisionata; il canale dati usa `curl` immediatamente dopo questo preflight perché `FtpWebRequest` su Linux lascia file a zero byte quando Altervista restituisce `451`. I file applicativi testuali vengono trasferiti in modalità FTP ASCII, coerentemente con il comportamento automatico di FileZilla, e verificati dopo il download normalizzando esclusivamente i fine-riga. Il deploy applica retry a ogni operazione, ma Altervista non offre un equivalente automatico di `app_offline.htm`: in caso di errore dopo un trasferimento parziale, il workflow segnala quante operazioni sono state completate e richiede una verifica esplicita prima del retry.
