...
$requestBody = preg_replace('/^\xEF\xBB\xBF/', '', file_get_contents('php://input') ?: '');
...
if (!is_array($request)) {
    http_response_code(400);
    echo json_encode(['error' => 'JSON request must be an object.']);
    exit;
}
...
if (!$clearAlbumRoutingCache && !$clearPhotoRoutingCache && !$clearApiResponseCache) {
    http_response_code(400);
    echo json_encode(['error' => 'At least one cache must be selected.']);
    exit;
}
