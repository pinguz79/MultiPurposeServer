# Portfolio.Web Roadmap

> **Stato: Alpha 0 — non autorevole.** Il contenuto deve essere verificato e trasferito nei registri e nelle roadmap consolidate.

## SEO e contenuti

### Obiettivo

Trasformare Portfolio.Web da semplice galleria fotografica a sito con contenuti editoriali originali, migliorando:

- indicizzazione nei motori di ricerca;
- approvazione Google AdSense;
- tempo di permanenza degli utenti;
- valore informativo delle pagine.

---

## Arricchimento delle pagine

### Album

- [ ] Descrizione estesa dell'album.
- [ ] Introduzione allo shooting.
- [ ] Informazioni sulla location.
- [ ] Categoria fotografica.
- [ ] Tag SEO.

### Foto

- [ ] Descrizione della fotografia.
- [ ] Titolo descrittivo.
- [ ] Testo alternativo (alt).
- [ ] Didascalia.
- [ ] Keywords.

### Dati tecnici

- [ ] Fotocamera.
- [ ] Obiettivo.
- [ ] Lunghezza focale.
- [ ] Tempo di esposizione.
- [ ] Diaframma.
- [ ] ISO.
- [ ] Flash.
- [ ] Data dello scatto.

---

## SEO tecnico

- [ ] Meta description dedicate.
- [ ] Open Graph.
- [ ] Twitter Cards.
- [ ] Sitemap immagini.
- [ ] Breadcrumb.
- [ ] URL canoniche.
- [ ] Schema.org ImageObject.
- [ ] Miglioramento heading H1/H2/H3.

---

## Contenuti editoriali

Valutare l'introduzione di:

- [ ] Articoli sugli shooting.
- [ ] Backstage.
- [ ] Consigli fotografici.
- [ ] Presentazione delle modelle (quando autorizzato).
- [ ] Racconti degli shooting.
- [ ] Articoli tecnici sull'attrezzatura.

---

## Google AdSense

Obiettivo:

Aumentare il contenuto testuale e il valore informativo del sito per soddisfare i requisiti qualitativi di Google AdSense, evitando che Portfolio.Web venga percepito come una semplice raccolta di immagini.
